...
.brand-logo{display:block;width:270px;max-width:42vw;height:auto;border-radius:12px;background:#fff;padding:4px;box-shadow:0 10px 30px rgba(2,9,17,.28)}
@media(max-width:900px){.top{justify-content:center}.brand-logo{width:250px;max-width:68vw}}
@media(max-width:520px){.brand-logo{width:220px;max-width:76vw;border-radius:10px;padding:3px}}
`;
  ...
  args[listenerIndex] = function brandedListener(req, res) {
    ...
    if ((method === 'GET' || method === 'HEAD') && (pathname === '/' || pathname === '/index.html')) {
      ...
      fs.readFile(indexPath, 'utf8', (err, html) => {
        ...
        let output = html;
        if (!output.includes('brand-logo.webp')) {
          output = output.replace(
            '<div class="brand">🚘 Consulta Veicular 360</div>',
            '<div class="brand"><a class="brand-link" href="/" aria-label="Consulta Veicular 360"><img class="brand-logo" src="/brand-logo.webp?v=1" alt="Consulta Veicular 360" width="780" height="260"></a></div>'
          );
