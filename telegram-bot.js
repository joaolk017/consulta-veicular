...
async function sendWelcome(chatId){
  ...
  const artworkCompact=path.join(__dirname,"a_slick_high_contrast_promotional_banner_graphic.png");
  const artworkNew=path.join(__dirname,"telegram-welcome-new.png");
  const artwork=fs.existsSync(artworkCompact)?artworkCompact:fs.existsSync(artworkNew)?artworkNew:fs.existsSync(artworkInAssets)?artworkInAssets:path.join(__dirname,"telegram-welcome.png");
