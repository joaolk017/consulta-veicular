...
app.use((req, res, next) => {
  ...
  if (pathname === "/escolha-360.js" || pathname === "/assistente-compra.js" || pathname === "/service-worker.js") {
...
function optimizeComplete360VerifiedV3() {
  ...
  const target=new Set(groups);
  // Coberturas e preços confirmados na documentação CredPro em 20/09/2026.
  // O catálogo vivo GET /v1/pesquisas/itens continua sendo a fonte da verdade antes de produção.
  const products=[
  ...
  let best=null,n=products.length;
  for(let mask=1;mask<(1<<n);mask++){
    ...
    const missing=groups.filter(g=>!covered.has(g));
    const cand={complete:missing.length===0,coveredCount:covered.size,cost,covered,missing,selected};
  ...
  const pricing=complete?[.45,.50,.55,.60].map(m=>({targetMarginPercent:m*100,minimumSalePrice:Number((apiCost/(1-PAYMENT_FEE_RATE-m)).toFixed(2))})):[];
  return {mode:"complete_360_verified_v3",apiCallsMade:0,catalogDate:"2026-09-20",complete,coveredCount:best.coveredCount,totalGroups:groups.length,missingIds:best.missing,selected:best.selected.map(p=>({provider:p.provider,item:p.item,price:p.price,covers:p.covers})),estimatedApiCost:apiCost,targetApiCostRange:{min:10,max:15},withinTargetCost:complete&&apiCost>=10&&apiCost<=15,pricing,saleAllowedAsComplete:complete,note:"Cálculo local com catálogo documentado; nenhuma pesquisa veicular foi executada. Validar preços no GET /v1/pesquisas/itens antes de produção."};
}
async function runComplete360VerifiedV3Once(){
 try{
  requireDatabase();await pool.query(`CREATE TABLE IF NOT EXISTS admin_one_time_actions (action_key TEXT PRIMARY KEY, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
  const q=await pool.query("INSERT INTO admin_one_time_actions(action_key) VALUES($1) ON CONFLICT(action_key) DO NOTHING RETURNING action_key",["complete-360-verified-v3"]);
  if(q.rowCount)console.log("COMPLETE 360 OPTIMIZER V3:",JSON.stringify(optimizeComplete360VerifiedV3()));
...
function calculateSafeApiCaps(targetMarginPercent = 55) {
  const target = Math.max(0, Math.min(95, Number(targetMarginPercent))) / 100;
  const rows = Object.values(PACKAGES).map(p => {
    const revenue = Number(p.amount);
    const paymentFee = revenue * PAYMENT_FEE_RATE;
    const maxTotalApiCost = Math.max(0, revenue - paymentFee - (revenue * target));
    return {
      product:p.id,
      credits:p.credits,
      revenue:Number(revenue.toFixed(2)),
      targetMarginPercent:Number((target*100).toFixed(1)),
      maxTotalApiCost:Number(maxTotalApiCost.toFixed(2)),
      maxApiCostPerConsult:Number((maxTotalApiCost/p.credits).toFixed(2))
    };
  });
  return {
    targetMarginPercent:Number((target*100).toFixed(1)),
    rows,
    note:"Teto gerencial calculado para preservar a margem de contribuição alvo; não autoriza nem executa chamadas de API."
  };
}
...
async function runStoredCoverageDryRunOnStartupOnce() {
  try {
    ...
    const relevantIds = Object.keys(labelById);
    const availableIds = relevantIds.filter(id => detected[id] === true);
    const missingIds = relevantIds.filter(id => detected[id] !== true);
    const available = availableIds.map(id => labelById[id]);
    const missing = missingIds.map(id => labelById[id]);
...
async function initTelegramOrders(){
  ...
  telegramBot.configureTelegramPayments({start:startTelegramPurchase,status:telegramOrderStatus,list:telegramOrderList,recover:telegramRecoverPix,cancel:telegramCancelOrder});
...
async function telegramCancelOrder(chatId,index){
  const order=await telegramOrderByIndex(chatId,index);
  if(!order||order.status!=="pending"){await telegramBot.sendTelegramMessage(chatId,"Esse pedido não está mais disponível para cancelamento.");return;}
  const claim=await pool.query("UPDATE telegram_orders SET status='cancelling',updated_at=NOW() WHERE correlation_id=$1 AND chat_id=$2 AND status='pending' RETURNING correlation_id",[order.correlation_id,String(chatId)]);
  if(!claim.rowCount){await telegramBot.sendTelegramMessage(chatId,"O status deste pedido mudou. Consulte Meus pedidos.");return false;}
  try{
    const charge=await getOpenPixCharge(order.correlation_id);
    const status=String(charge.status||"").toUpperCase();
    if(status==="COMPLETED"){
      await pool.query("UPDATE telegram_orders SET status='pending',updated_at=NOW() WHERE correlation_id=$1 AND status='cancelling'",[order.correlation_id]);
      await telegramBot.sendTelegramMessage(chatId,"Este PIX já foi pago. Não é possível cancelar; o relatório será processado.");return false;
    }
    if(status!=="ACTIVE"&&status!=="EXPIRED")throw new Error("Status da cobrança não permite cancelamento.");
    if(status==="ACTIVE"){
      const providerID=String(charge.globalID||charge.id||charge.identifier||"").trim();
      if(!providerID||providerID.length>300)throw new Error("Identificador da cobrança indisponível.");
      const response=await requestJson(WOOVI_API_URL+"/charge/"+encodeURIComponent(providerID),{method:"DELETE",headers:openPixHeaders(),timeout:15000});
      if(response.status<200||response.status>=300)throw new Error("A Woovi não confirmou o cancelamento (HTTP "+response.status+").");
    }
    await pool.query("UPDATE telegram_orders SET status='cancelled',updated_at=NOW() WHERE correlation_id=$1 AND status='cancelling'",[order.correlation_id]);
    await telegramBot.sendTelegramMessage(chatId,"🚫 Pedido da placa "+order.plate+" cancelado. Não utilize o PIX antigo.",{inline_keyboard:[[{text:"🔄 Nova consulta",callback_data:"placa"}],[{text:"🏠 Menu principal",callback_data:"menu"}]]});
    return true;
  }catch(err){
    await pool.query("UPDATE telegram_orders SET status='pending',updated_at=NOW() WHERE correlation_id=$1 AND status='cancelling'",[order.correlation_id]);
    console.error("Telegram cancelamento:",err.message);
    await telegramBot.sendTelegramMessage(chatId,"Não foi possível confirmar o cancelamento na Woovi. O pedido permanece em verificação; não pague o PIX até conferir o status.");
    return false;
  }
}
